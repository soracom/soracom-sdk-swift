...
extension Request where T == NoResponseBody {
    ...
    // FIXME: REVIEW THIS: Is this not part of the API spec (and if so, how did it get i here? 🤔
    
    /// Add (agree to terms and sign up for) another coverage type to existing operator.
    
    //    public class func addCoverageType(_ coverageType: CoverageType, operatorId: String, responseHandler: ResponseHandler<NoResponseBody>? = nil) -> Request<NoResponseBody> {
    //
