// SessionStatus.swift Generated by swagger-codegen ___SORACOM_SDK_SWIFT_SWAGGERER_CODE_GENERATION_METADATA___

import Foundation


public class SessionStatus: PayloadConvertible {

    public var isOnline: Bool?
    public var lastUpdatedAt: Int64?
    public var imei: String?
    public var ueIpAddress: String?
    public var dnsServers: [String]?
    public var vpgId: String?
    public var gatewayPrivateIpAddress: String?
    public var gatewayPublicIpAddress: String?

    public required init(
        isOnline: Bool? = nil, 
        lastUpdatedAt: Int64? = nil, 
        imei: String? = nil, 
        ueIpAddress: String? = nil, 
        dnsServers: [String]? = nil, 
        vpgId: String? = nil, 
        gatewayPrivateIpAddress: String? = nil, 
        gatewayPublicIpAddress: String? = nil
    ) {
        self.isOnline = isOnline
        self.lastUpdatedAt = lastUpdatedAt
        self.imei = imei
        self.ueIpAddress = ueIpAddress
        self.dnsServers = dnsServers
        self.vpgId = vpgId
        self.gatewayPrivateIpAddress = gatewayPrivateIpAddress
        self.gatewayPublicIpAddress = gatewayPublicIpAddress
        
    }

    // MARK: PayloadConvertible

    public func toPayload() -> Payload {

        let payload: Payload = [:]

        payload[.isOnline] = isOnline
        payload[.lastUpdatedAt] = lastUpdatedAt
        payload[.imei] = imei
        payload[.ueIpAddress] = ueIpAddress
        payload[.dnsServers] = dnsServers
        payload[.vpgId] = vpgId
        payload[.gatewayPrivateIpAddress] = gatewayPrivateIpAddress
        payload[.gatewayPublicIpAddress] = gatewayPublicIpAddress

        return payload;
    }

    public static func from(_ payload: Payload?) -> Self? {
        guard let payload = payload else {
            return nil
        }

        _ = payload // suppress compiler warnings when payload isn't used in generated code

        let result = self.init()

        result.isOnline = payload.getBool(.isOnline)
        result.lastUpdatedAt = payload.getInt64(.lastUpdatedAt)
        result.imei = payload.getString(.imei)
        result.ueIpAddress = payload.getString(.ueIpAddress)
        result.dnsServers = payload.getStringArray(.dnsServers)
        result.vpgId = payload.getString(.vpgId)
        result.gatewayPrivateIpAddress = payload.getString(.gatewayPrivateIpAddress)
        result.gatewayPublicIpAddress = payload.getString(.gatewayPublicIpAddress)
        return result
    }

}

public typealias SessionStatusList = [SessionStatus]
