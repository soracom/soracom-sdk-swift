...
class BaseTestCaseTests: BaseTestCase {
    ...
    func test_isEquivalentJSON_some_more() {
        ...
        XCTAssertTrue(isEquivalentJSON(left, right))
        XCTAssertTrue(isEquivalentJSON(right, left))
        ...
        XCTAssertFalse(isEquivalentJSON(left, wrong))
        XCTAssertFalse(isEquivalentJSON(wrong, left))
