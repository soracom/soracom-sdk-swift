...
class Swaggerer {
    ...
    func runSwaggerCommand() -> Bool {
        ...
        let swag = TaskWrapper(pathToSwaggerCodegen, arguments: ["generate", "-i", inputFile, "-l", "swift", "-o", intermediateDirectory, "-t", templatesDir, "--type-mappings", "integer=Int64,long=Int64"])
        ...
        // can't work with the _ character for that option. In the end, though, I don't think we need to do that anyway.
    ...
    func postProcessGeneratedCode() -> Bool {
        ...
        let unwanted = [
            ...
            "SwaggerClient/Classes/Swaggers/Models/Mapstringstring.swift",
            "SwaggerClient/Classes/Swaggers/Models/MapstringDataTrafficStats.swift",
              // these are problematic with the code-gen, skipping these for now until I figure out how to handle
        ]
        var copied: [String] = []
        ...
        for subpath in e {
            ...
            } else if unwanted.contains(subpath) {
                shouldCopy = false
        ...
        let swaggererCodeGenerationMetadata = " — Custom output templates and post-processing by CodeGeneratorForSoracomSDKSwift.Swaggerer version " + swaggererVersion + ". Generated " + ISO8601DateFormatter().string(from: Date())
        
        let tr = TextReplacer()
        let replacements = [
            "___SWAGGERER_POSTPROCESS_FIXME_[String]___"        : "getStringArray",
            "___SWAGGERER_POSTPROCESS_FIXME_[String:String]___" : "decodeStringToStringDictionary",
            "___SWAGGERER_POSTPROCESS_FIXME_[ActionConfig]___"  : "decodeActionConfig",
            "___SWAGGERER_POSTPROCESS_FIXME_[OrderItemModel]___" : "decodeOrderItemModel",
            "___SWAGGERER_POSTPROCESS_FIXME_[TagUpdateRequest]___" : "decodeTagUpdateRequest",
            "___SWAGGERER_POSTPROCESS_FIXME_[String:DataTrafficStats]___" : "decodeStringToDataTrafficStatsDictionary",
            "___SWAGGERER_POSTPROCESS_FIXME_[String:SoracomBeamStats]___" : "decodeStringToSoracomBeamStatsDictionary",
            "___SWAGGERER_POSTPROCESS_FIXME_[ProductModel]___"            : "decodeProductModel",
            "___SWAGGERER_POSTPROCESS_FIXME_[ShippingCostModel]___"       : "decodeShippingCostModel",
            "___SWAGGERER_POSTPROCESS_FIXME_[CouponResponse]___"          : "decodeCouponResponse",
            "___SWAGGERER_POSTPROCESS_FIXME_[AuthKeyResponse]___"         : "decodeAuthKeyResponse",
            "___SWAGGERER_POSTPROCESS_FIXME_[ListRolesResponse]___"       : "decodeListRolesResponse",
            "___SWAGGERER_POSTPROCESS_FIXME_[GetShippingAddressResponse]___" : "decodeGetShippingAddressResponse",
            "___SWAGGERER_POSTPROCESS_FIXME_[EstimatedOrderItemModel]___" : "decodeEstimatedOrderItemModel",
            "___SWAGGERER_POSTPROCESS_FIXME_[GetOrderResponse]___"        : "decodeGetOrderResponse",
            "___SWAGGERER_POSTPROCESS_FIXME_[DailyBill]___"               : "decodeDailyBill",
            "___SWAGGERER_POSTPROCESS_FIXME_[OrderedSubscriber]___"       : "decodeOrderedSubscriber",
            "___SWAGGERER_POSTPROCESS_FIXME_[MonthlyBill]___"             : "decodeMonthlyBill",
            
            "case S1.minimum =" : "case s1_minimum  =",
            "case S1.slow ="    : "case s1_slow     =",
            "case S1.standard =": "case s1_standard =",
            "case S1.fast ="    : "case s1_fast     =",
            
            "gettype" : "getType",
            "decodetype" : "decodeType",
            
            "___SORACOM_SDK_SWIFT_SWAGGERER_CODE_GENERATION_METADATA___" : swaggererCodeGenerationMetadata
        ]
        let outputURL = URL(fileURLWithPath: outputDirectory)
        
        for (old, new) in replacements {
            let errs = tr.replaceOccurences(of: old, with: new, directory: outputURL, extensions: ["swift"])
            guard errs == nil else {
                print(errs?.joined(separator: "\n"))
                fatalError("post-process text replace failed")
            }
        }
